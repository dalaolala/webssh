...
- **本地历史**: 连接信息保存在浏览器本地
- **分组管理**: 自动按分组管理连接历史
- **安全导入导出**: 支持连接历史的加密导入导出，每次操作都需要输入密钥
- **无数据库**: 无需配置数据库，简化部署

### 导入导出功能

**安全特性：**
- 🔐 **每次操作都需要密钥**: 每次导入导出都必须输入加密密钥，确保数据安全
- 🔑 **密钥不保存**: 密钥不会保存在本地，用户需要自己妥善保管
- 🛡️ **AES加密**: 使用AES加密算法保护连接数据
- 📁 **安全文件格式**: 导出文件使用 `.enc` 扩展名，格式为 `webssh_quick_connect_YYYYMMDD.enc`

**使用流程：**
1. **导出**: 点击导出按钮 → 输入加密密钥（至少6位） → 下载加密文件
2. **导入**: 点击导入按钮 → 选择加密文件 → 输入正确的加密密钥 → 导入成功

**注意事项：**
- ⚠️ 请务必妥善保管加密密钥，忘记密钥将无法解密导出的文件
- 🔒 建议使用强密码作为加密密钥
- 📱 导出文件可以在不同设备间安全传输
